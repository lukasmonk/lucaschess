...
extern "C" void lc_inicio(void)
{
	dataInit();
	board.init();
}

extern "C" int lc_check(void)
{
	if (board.nextMove)
       {
...
extern "C" void lc_setupFen(char *fen, char *fencolor, char *fencastling, char *fenenpassant, int fenhalfmoveclock, int fenfullmovenumber)
{
...
extern "C" void lc_makemove(int pos)
{
...
extern "C" char * lc_toSAN(int pos)
{
...
extern "C" char * lc_getPosition(void)
{
	int i;
	int piece, pos, d;
	int asciiShift, asciiDec;
...
extern "C" char * lc_getFen(void)
{
	int rank, file;
	int nSin, piece, pos, i;
	int asciiShift, asciiDec;
	char tmp[20];
	...
	// Pawnmoves + Full moves
	lcBuffer[pos++] = ' ';

	sprintf( tmp, "%d %d", board.fiftyMove, board.fullmovenumber );
...
extern "C" char * lc_pgn2pv(char * pgn)
{
