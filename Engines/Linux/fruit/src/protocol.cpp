...
startTime = time(NULL);    //= WallClockTime(); // time stamp (millisec)
startCpuTime  = clock();      //    = CPUtime(); // Total CPU used upto now (millisec)
send("info string times @ %lu\n", startTime); 
...
static void send_best_move() {
   ...
   my_time = SearchCurrent->time;
   speed = SearchCurrent->speed;
   cpu = SearchCurrent->cpu;
   node_nb = SearchCurrent->node_nb;

  send("info time %.0f nodes " S64_FORMAT " nps %.0f cpuload %.0f",my_time*1000.0,node_nb,speed,cpu*1000.0);
   ...
   if (pv[0] == move && move_is_ok(pv[1])) {
	  ...
	  stopTime = time(NULL);               //  = WallClockTime(); // time stamp
      stopCpuTime  = clock();              // = CPUtime() - cpuTime; // Consumed CPU time
      
	  
	   
	 diff_time  = stopTime - startTime;
     diff_clock = stopCpuTime - startCpuTime;
      
	  
	  
	  
	  send("info string times @ %lu: real=%lu cpu=%.3f sec\n", stopTime, diff_time, (double)diff_clock/CLOCKS_PER_SEC);  
	 ...
	 stopTime = time(NULL);             // = WallClockTime(); // time stamp
     stopCpuTime  = clock();            // Consumed CPU time
