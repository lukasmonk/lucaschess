...
bool input_available() {
   ...
   if (!init) {
      ...
      if (UseDebug && (stdin_h == NULL || stdin_h == INVALID_HANDLE_VALUE)) {
         ...
         printf("info string GetStdHandle() failed, error=%d\n",error);
            if (UCI_debug && fp_debug != NULL) {
                fprintf(fp_debug,"info string GetStdHandle() failed, error=%d\n\r",error);
            }
     }
      ...
      if (UseDebug) printf("info string init=%d is_pipe=%d\n",int(init),int(is_pipe));
            if (UCI_debug && fp_debug != NULL) {
                fprintf(fp_debug,"info string init=%d is_pipe=%d\n\r",int(init),int(is_pipe));
            }
      ...
      if (UseDebug && is_pipe) { // GetConsoleMode() failed, everybody assumes pipe then
         ...
         printf("info string GetConsoleMode() failed, error=%d\n",error);
            if (UCI_debug && fp_debug != NULL) {
                fprintf(fp_debug,"info string GetConsoleMode() failed, error=%d\n\r",error);
            }
      }
   ...
   if (is_pipe) {
      ...
      if (!PeekNamedPipe(stdin_h,NULL,0,NULL,&val,NULL)) {
         ...
         if (UseDebug) { // PeekNamedPipe() failed, everybody assumes EOF then
            ...
            if (UCI_debug && fp_debug != NULL) {
                fprintf(fp_debug,"info string PeekNamedPipe() failed, error=%d\n\r",error);
            }
         }
      ...
      if (UseDebug && val < 0) {
          printf("info string PeekNamedPipe(): val=%d\n",val);
            if (UCI_debug && fp_debug != NULL) {
                fprintf(fp_debug,"info string PeekNamedPipe(): val=%d\n\r",val);
            }
      }
