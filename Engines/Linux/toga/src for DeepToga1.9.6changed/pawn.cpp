...
typedef pawn_info_t pawn_entry_t;
...
void pawn_alloc() {
   ...
   if (UseTable) {
       ...
       for (ThreadId = 0; ThreadId < NumberThreadsInternal; ThreadId++){
           ...
           Pawn[ThreadId]->table = (pawn_entry_t *) my_malloc(Pawn[ThreadId]->size*sizeof(pawn_entry_t));
...
void pawn_clear(int ThreadId) {
   ...
   if (Pawn[ThreadId]->table != NULL) {
      memset(Pawn[ThreadId]->table,0,Pawn[ThreadId]->size*sizeof(pawn_entry_t));
   }
...
static void pawn_comp_info(pawn_info_t * info, const board_t * board) {
   ...
   int last_file, island, num_iso[ColourNb];
...
/*       // this was moved to the dynamic evaluation
   ...
   // Pawn Islands by Jerry Donald

   for (colour = 0; colour < ColourNb; colour++) {
       last_file = FileNone;
       island = 0;
       for (file = 0; file < FileNb; file++){
           if (board->pawn_file[colour][file] == 0 && file != FileA && file != FileH){
              if (file - 1 != last_file){
                       island++;
              }
              last_file = file;
       ...
       opening[colour] -= (island - num_iso[colour]) * IsolatedOpening; // 10
       endgame[colour] -= (island - num_iso[colour]) * IsolatedEndgame; // 20
   }    
   
   // TODO: advancement credit for island versus island with pawn majority...
   // OR: upgrade the pawn PST stuff.  perturb_pawns().  
