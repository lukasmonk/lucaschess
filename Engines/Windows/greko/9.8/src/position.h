...
public:
  ...
  FLD From() const { return mm_data & 0x3f; }
  FLD To() const { return (mm_data >> 6) & 0x3f; }
  FLD Piece() const { return (mm_data >> 12) & 0x0f; }
  FLD Captured() const { return (mm_data >> 16) & 0x0f; }
  FLD Promotion() const { return (mm_data >> 20) & 0x0f; }

  operator U32() const { return mm_data; }
...
public:
  ...
  FLD   FindLeastAttacker(FLD to, COLOR side, const U64& mask) const;
...
inline void Position::Put(FLD f, PIECE p)
{
  ...
  m_bitsAll[GetColor(p)] ^= BB_SINGLE[f];
  m_hash ^= s_hash[f][p];
