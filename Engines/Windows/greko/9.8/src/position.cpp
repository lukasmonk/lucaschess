...
{ 
  int total = 1;
  for (int i = _undoSize - 1; i >= 0; --i)
  {
    if (_undos[i].m_hash == m_hash) 
    {
      ++total;
    }
    ...
    if (_undos[i].m_mv.Captured())
      break;

    if (_undos[i].m_mv.Piece() == PW)
      break;

    if (_undos[i].m_mv.Piece() == PB)
      break;
   }
...
bool Position::IsAttacked(FLD to, COLOR side) const
{
  ...
  U64 occ = BitsAll();
  x = BB_BISHOP_ATTACKS[to] & (Bits(QW | side) | Bits(BW | side));
  while (x)
  {
    from = PopLSB(x);
  ...
  x = BB_ROOK_ATTACKS[to] & (Bits(QW | side) | Bits(RW | side));
  while (x)
  {
    from = PopLSB(x);
    if ((BB_BETWEEN[from][to] & occ) == 0)
...
void Position::Print() const
{
  ...
  {
    out("\n ");
    for (int m = 0; m < _undoSize; ++m)
      out("%s ", MoveToStrLong(_undos[m].m_mv));
    out("\n ");
  }
  out("\n");
}
