...
static const bool BIG_board = false;
...
void board_init_list(board_t * board) {
   ...
   board->moving_piece = PieceNone256; // Thomas for bitbases
...
//   board_repetitions   (const board_t * board);

int  board_repetitions   (const board_t * board)  {
   ...
   for (i = 4; i <= board->ply_nb; i += 2) {
       ...
       if (board->stack[board->sp - i] == board->key) reps++;
   }
...
extern void fprint_board(const board_t * board, FILE * fpointer)
   {
   char board_chars[64];
   ...
   if (BIG_board) // initialize with blanks.
      {
      for (i = 0; i < 64; i++)
          {
          if (((i+i/8)&1)==1) board_chars[i] = ' ';
          else                board_chars[i] = ' ';
          }
      }
   else           // initialize with dashes.
      {
      for (i = 0; i < 64; i++)
          {
          if (((i+i/8)&1)==1) board_chars[i] = '-';
   ...
   // supplant the white pieces.

   ASSERT(PIECE_IS_KING(board->square[board->piece[White][0]]));

   for (ptr = &board->piece[White][0]; (square=*ptr) != SquareNone; ptr++)
       {
       piece = board->square[square]; // the things on the square

       i = SQUARE_TO_64(square);
       ...
       if      (PIECE_IS_BISHOP(piece)) board_chars[i]='B';
       else if (PIECE_IS_KNIGHT(piece)) board_chars[i]='N';
       else if (PIECE_IS_ROOK(piece))   board_chars[i]='R';
       else if (PIECE_IS_QUEEN(piece))  board_chars[i]='Q';
       else if (PIECE_IS_KING(piece))   board_chars[i]='K';
       }


   // supplant the white pawns.

   for (ptr = &board->pawn[White][0]; (square=*ptr) != SquareNone; ptr++)
       {
       piece = board->square[square]; // the things on the square
       ...
       if      (PIECE_IS_PAWN(piece))   board_chars[i]='P';

       ASSERT (!PIECE_IS_BISHOP(piece));
       ASSERT (!PIECE_IS_KNIGHT(piece));
       ASSERT (!PIECE_IS_ROOK(piece));
       ASSERT (!PIECE_IS_QUEEN(piece));
       ASSERT (!PIECE_IS_KING(piece));
       }

   // supplant the Black pieces.

   ASSERT(PIECE_IS_KING(board->square[board->piece[Black][0]]));

   for (ptr = &board->piece[Black][0]; (square=*ptr) != SquareNone; ptr++)
       {
       piece = board->square[square]; // the things on the square
   ...
   // supplant the Black pawns.

   for (ptr = &board->pawn[Black][0]; (square=*ptr) != SquareNone; ptr++)
       {
       piece = board->square[square]; // the things on the square
   ...
   if (BIG_board)
      ...
      for (row = 0; row < 8; row++)
          ...
          for (int col = 0; col < 8; col++)
              ...
              fprintf(fpointer, "|  %c  ",board_chars[i]);
             }
