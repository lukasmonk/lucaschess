...
// filas pieza casilla
extern int Precalc_CMf[6][64];
// columnas pieza casilla
extern int Precalc_CMc[6][64];
...
public:
	...
	int Quiesce(int alpha,int beta);//, char *Global);
	void MuevePath(char *path);

	void Cancela();
	void LoadEPD(char *fen);
// mejor Path encontrado en el paso anterior
	void SetMejorPath(char *path);
public:
...
private:
	...
	void GotoPath(char *path,int depth);
