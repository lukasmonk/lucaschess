...
uint8 tt_probe(int depth, int *value, int beta, uint32 *h_move, uint8 *null_ok, int nmR)
{
  ...
  h_entry = tt + (pos->hash & opt.tt_numentries);
	
  for(i = 0; i < BUCKET_SIZE; i++)
  { if((h_entry+i)->key == pos->hash)
      break;
  }
  if(i < BUCKET_SIZE)
  { h_entry += i;
    ...
    *h_move = h_entry->move;
    ...
    { *value = h_entry->value;
      //age update:
      h_entry->flag = (age << 4) | (h_entry->flag & 0x0F);
...
void tt_save(int depth, int value, uint8 flag, uint32 h_move)
{
  ...
  si.num_hash_saves++;
  h_replace->key   = pos->hash;
  h_replace->value = (sint16)value;
  h_replace->depth = (uint8)depth;
  h_replace->flag  = (age << 4) | flag;
  h_replace->move = h_move;
}
...
{///getting the hash move for the current pos.
  int i;
  tt_t *h_entry = tt + (pos->hash & opt.tt_numentries);
...
///test the move if it's legal in order to
///avoid bogus output.
  int result = 0;
  move_t move;
  move_make(si.rootmove);
  move.p = tt_get_hashmove();
  if(move.p && is_legal(move))
  { m->p = move.p;
    result = 1;
  }
  move_unmake();
