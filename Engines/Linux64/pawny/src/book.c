...
const int piece_code[16] = 
{0, 1, 3, 5, 7, 9, 11, 0, 0, 0, 2, 4, 6, 8, 10, 0};
...
static __inline uint64 book_hash_position()
{
  ...
  for(sq = 0; sq < 64; sq++)
  { if(Piece(sq) == EMPTY) continue;
    key ^= book_rnd[64 * piece_code[Piece(sq)] + 8 * Rank(sq) + File(sq)];
  }
  if(pos->castle & WHITE_OO)  key ^= book_rnd[768];
  if(pos->castle & WHITE_OOO) key ^= book_rnd[769];
  if(pos->castle & BLACK_OO)  key ^= book_rnd[770];
  if(pos->castle & BLACK_OOO) key ^= book_rnd[771];
  if(pos->ep) 
  { sq = pos->ep;
    if(sq == EMPTY)//'is_ep_possible':
    { if(pos->side == B)
      {	if(Piece(sq + 8) == WP)
        { if(File(sq) == FILE_A)
          { if(Piece(sq + 9) == BP)
              key ^= book_rnd[772 + File(sq)];	
          }
          else if(File(sq) == FILE_H)
          { if(Piece(sq + 7) == BP)
              key ^= book_rnd[772 + File(sq)];	
          }
          else
          { if((Piece(sq + 9) == BP) || (Piece(sq + 7) == BP))
              key ^= book_rnd[772 + File(sq)];	
          }
        }
      }
      ...
      {	if(Piece(sq - 8) == BP)
        { if(File(sq) == FILE_A)
          ...
          { if((Piece(sq - 9) == WP) || (Piece(sq - 7) == WP))
              key ^= book_rnd[772 + File(sq)];	
          }
        }
      }
  ...
  if(pos->side == WHITE) key ^= book_rnd[780];
  return key;
}
...
static __inline bool read_entry(book_entry_t *pentry, int p)
{ 
  if((fseek(book_file,(p * sizeof(book_entry_t)), SEEK_SET)) == -1) 
    return false;
...
{ 
  ...
  m->from = (uint8)CalcSq(from_file, from_rank);
  m->to = (uint8)CalcSq(to_file, to_rank);
  //(prom + 1) is the uncolored piece type
  m->promoted = (prom) ? (uint8)Coloured((prom + 1), pos->side) : (0);
  
  //then check the move if it exists in the ms:
  move_count = move_gen_legal(ms);
