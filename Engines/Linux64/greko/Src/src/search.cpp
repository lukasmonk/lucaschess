...
EVAL Search::AlphaBetaRoot(EVAL alpha, EVAL beta, const int depth)
{
	...
	if (pentry) hashMove = pentry->m_mv;
	...
	for (int i = 0; i < mvlist.Size(); ++i)
	{
		...
		if (m_pos.MakeMove(mv))
		{
			...
			if (e == DRAW_SCORE) e = g_evalParams.DrawScore;
...
EVAL Search::AlphaBeta(EVAL alpha, EVAL beta, const int depth, int ply, bool isNull)
{
	...
	if (!inCheck && !onPV && depth > 0 && depth < 4)
	{
		...
		if (staticScore <= alpha - MARGIN[depth]) return AlphaBetaQ(alpha, beta, ply, 0);
		if (staticScore >= beta + MARGIN[depth]) return beta;
	}
		...
		if (m_pos.MakeMove(mv))
		{
			...
			if (!onPV && !inCheck && !givesCheck && !mv.Captured() && !mv.Promotion())
			{
				if (depth >= g_searchParams.LmrMinDepth &&
					legalMoves >= g_searchParams.LmrMinMoveNumber)
				{
...
EVAL Search::AlphaBetaQ(EVAL alpha, EVAL beta, int ply, int qply)
{
	...
	{
		Move mv = mvlist.GetNthBest(i);
		if (!inCheck && SEE(m_pos, mv) < 0) continue;
		if (m_pos.MakeMove(mv))
		{
			legalMoves++;
	...
	if (inCheck && legalMoves == 0) alpha = -CHECKMATE_SCORE + ply;
	return alpha;
}

void Search::CheckInput()
{
	if (m_rootPV.empty()) return;
	if (InputAvailable())
	{
		char buf[4096];
...
void Search::CheckLimits()
{
	...
	{
		if (m_mode == THINKING || m_mode == EPDTEST) m_flag = true;
	}
}
